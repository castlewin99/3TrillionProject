...
public class PageService {
	public static String printPage(String url, int totalCount, int pageBlock, int currentPage) {
		...
		if(currentPage > 1)
			result += "<a href='" + url + (currentPage-1)+ "class=\"h2 text-uppercase mb-2\"'> 이전 </a>";
		else
		...
		if(currentPage < totalPage)
			result += "<a href='" + url + (currentPage+1)+ "class=\"h2 text-uppercase mb-2\"'> 다음 </a>";
		else
			result += "<a href='" + url + totalPage+"class=\"h2 text-uppercase mb-2\"'> 다음 </a>";
